...
union amd_viommu_ucmd_buffer {
	...
	struct amd_viommu_gcr3_data gcr3;
};
...
int viommu_gcr3_update(struct iommufd_ucmd *ucmd)
{
	struct amd_viommu_gcr3_data *data = ucmd->cmd;

	return amd_viommu_gcr3_update(data);
}
...
struct iommufd_ioctl_op viommu_ioctl_ops[] = {
	...
	IOCTL_OP(VIOMMU_GCR3_UPDATE, viommu_gcr3_update,
		 struct amd_viommu_gcr3_data, gcr3_va),
};
