...
int amd_viommu_domain_id_update(struct iommu_hwpt_amd_v2 *hwpt, bool is_set)
{
	...
	hdev_id = viommu_get_hdev_id(iommu, gid, hwpt->gdev_id);
	hdom_id = dev_table[hdev_id].data[1] & 0xFFFFULL;

	if (is_set) {
		set_domain_mapping(iommu, gid, hdom_id, hwpt->gdom_id);
		dump_domain_mapping(iommu, gid, hwpt->gdom_id);
	} else
		clear_domain_mapping(iommu, gid, hdom_id, hwpt->gdom_id);
