...
static void set_iommu_private_space_dte(struct amd_iommu *iommu)
{
	u64 pte_root = 0;
	u64 flags = 0;
	...
	pte_root |= (pdom->iop.mode & DEV_ENTRY_MODE_MASK) << DEV_ENTRY_MODE_SHIFT;
...
static int viommu_private_space_init(struct amd_iommu *iommu)
{
	...
	set_iommu_private_space_dte(iommu);
...
int __init iommu_init_viommu(struct amd_iommu *iommu)
{
	...
	if (!check_feature(FEATURE_VIOMMU) ||
	    !check_feature2(FEATURE_GCR3TRPMODESUP))
		goto err_out;
...
int amd_viommu_device_update(struct amd_viommu_dev_info *data, bool is_set)
{
	...
	if (is_set) {
		...
		amd_iommu_flush_tlb(to_pdomain(dom), 0);
...
int amd_viommu_user_gcr3_update(const void *user_data, struct iommu_domain *udom)
{
	...
	struct pci_dev *pdev;
	struct amd_iommu *iommu;
	struct iommu_hwpt_amd_v2 data, *hwpt = NULL;
	const size_t min_len = offsetofend(struct iommu_hwpt_amd_v2, gdev_id);

	if (!user_data)
		return -EINVAL;

	ret = iommu_copy_user_data(&data, user_data,
